...
constexpr VkDebugUtilsMessageTypeFlagsEXT debugType =
    0 | VK_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT |
...
// better not be used often because of coil whine
...
// Makes present queue from different Queue Family than Graphics, for testing
// purposes
constexpr bool forceSeparatePresentQueue = false;

// needed stuff for main() -- forward declarations
//////////////////////////////////////////////////////////////////////////////////
VkDescriptorSet createDescriptorSet(VkBuffer uniformBuffer,
...
createTextureImage(const char *imagePath, VkDevice device,
...
bool isLayerSupported(const char *layer,
                      const vector<VkLayerProperties> &supportedLayers);
bool isExtensionSupported(
    const char *extension,
    const vector<VkExtensionProperties> &supportedExtensions);
// treat layers as optional; app can always run without em -- i.e. return those
// supported
vector<const char *>
checkInstanceLayerSupport(const vector<const char *> &requestedLayers,
                          const vector<VkLayerProperties> &supportedLayers);
vector<VkExtensionProperties>
getSupportedInstanceExtensions(const vector<const char *> &providingLayers);
bool checkExtensionSupport(
    const vector<const char *> &extensions,
    const vector<VkExtensionProperties> &supportedExtensions);

void updateUniformBuffer(VkDeviceMemory uniformBufferMemory, VkDevice device);
VkInstance initInstance(const vector<const char *> &layers = {},
                        const vector<const char *> &extensions = {});
...
VkPhysicalDevice getPhysicalDevice(
    ...
    VkSurfaceKHR surface =
        VK_NULL_HANDLE /*seek presentation support if !NULL*/); // destroyed
                                                                // with instance
VkPhysicalDeviceProperties
getPhysicalDeviceProperties(VkPhysicalDevice physicalDevice);
VkPhysicalDeviceMemoryProperties
getPhysicalDeviceMemoryProperties(VkPhysicalDevice physicalDevice);

std::pair<uint32_t, uint32_t> getQueueFamilies(VkPhysicalDevice physDevice,
                                               VkSurfaceKHR surface);
vector<VkQueueFamilyProperties>
getQueueFamilyProperties(VkPhysicalDevice device);

VkDevice initDevice(VkPhysicalDevice physDevice,
...
VkBuffer initBuffer(VkDevice device, VkDeviceSize size,
                    VkBufferUsageFlags usage);
void killBuffer(VkDevice device, VkBuffer buffer);

VkImage initImage(VkDevice device, VkFormat format, uint32_t width,
...
VkSwapchainKHR initSwapchain(VkPhysicalDevice physicalDevice, VkDevice device,
                             VkSurfaceKHR surface,
...
vector<VkImageView> initSwapchainImageViews(VkDevice device,
                                            vector<VkImage> images,
                                            VkFormat format);
void killSwapchainImageViews(VkDevice device, vector<VkImageView> &imageViews);

VkRenderPass initRenderPass(VkDevice device, VkPhysicalDevice physicalDevice,
                            VkSurfaceFormatKHR surfaceFormat);
void killRenderPass(VkDevice device, VkRenderPass renderPass);

vector<VkFramebuffer> initFramebuffers(VkDevice device, VkRenderPass renderPass,
...
VkPipeline initPipeline(VkDevice device, VkPhysicalDeviceLimits limits,
                        VkPipelineLayout pipelineLayout,
                        VkRenderPass renderPass, VkShaderModule vertexShader,
                        VkShaderModule fragmentShader,
                        const uint32_t vertexBufferBinding, uint32_t width,
                        uint32_t height);
void killPipeline(VkDevice device, VkPipeline pipeline);

std::tuple<VkBuffer, VkDeviceMemory>
createUniformBuffer(VkDevice device, VkPhysicalDevice physicalDevice);

VkDescriptorPool createDescriptorPool(VkDevice device);
void setVertexData(VkDevice device, VkDeviceMemory memory,
                   vector<Vertex3D_UV> vertices);
...
vector<VkFence> initFences(VkDevice device, size_t count,
                           VkFenceCreateFlags flags = 0);
void killFences(VkDevice device, vector<VkFence> &fences);

void acquireCommandBuffers(VkDevice device, VkCommandPool commandPool,
                           uint32_t count,
...
void recordBeginRenderPass(VkCommandBuffer commandBuffer,
                           ...
                           const VkClearValue *clearValue, uint32_t width,
                           uint32_t height);
...
void recordBindVertexBuffer(VkCommandBuffer commandBuffer,
                            const uint32_t vertexBufferBinding,
                            VkBuffer vertexBuffer);

void recordDraw(VkCommandBuffer commandBuffer, uint32_t vertexCount);

void submitToQueue(VkQueue queue, VkCommandBuffer commandBuffer,
                   VkSemaphore imageReadyS, VkSemaphore renderDoneS,
                   VkFence fence = VK_NULL_HANDLE);
void present(VkQueue queue, VkSwapchainKHR swapchain,
             uint32_t swapchainImageIndex, VkSemaphore renderDoneS);
...
#if VULKAN_VALIDATION
  if (isLayerSupported("VK_LAYER_KHRONOS_validation", supportedLayers))
    requestedLayers.push_back("VK_LAYER_KHRONOS_validation");
  else
    throw "VULKAN_VALIDATION is enabled but VK_LAYER_KHRONOS_validation layers "
          "are not supported!";

  if (::useAssistantLayer) {
    if (isLayerSupported("VK_LAYER_LUNARG_assistant_layer", supportedLayers))
      requestedLayers.push_back("VK_LAYER_LUNARG_assistant_layer");
    else
      throw "VULKAN_VALIDATION is enabled but VK_LAYER_LUNARG_assistant_layer "
...
//                                         screenHeight, SDL_WINDOW_VULKAN);
  ...
  if (!SDL_Vulkan_GetInstanceExtensions(window, &requestedExtensionCount,
                                        nullptr)) {
    throw std::runtime_error("failed to get SDL Vulkan extensions");
  }
  ...
  if (!SDL_Vulkan_GetInstanceExtensions(window, &requestedExtensionCount,
                                        requestedInstanceExtensions.data())) {
    throw std::runtime_error("failed to get SDL Vulkan extensions");
  }
  ...
  if (true) {
    ...
    throw "VULKAN_VALIDATION is enabled but neither VK_EXT_debug_utils nor "
          "VK_EXT_debug_report extension is supported!";
#else
  uint32_t requestedExtensionCount = 0;
  if (!SDL_Vulkan_GetInstanceExtensions(window, &requestedExtensionCount,
                                        nullptr)) {
  ...
  // checkExtensionSupport(requestedInstanceExtensions,
  // supportedInstanceExtensions);

  const VkInstance instance =
      initInstance(requestedLayers, requestedInstanceExtensions);

#if VULKAN_VALIDATION
  const auto debugHandle =
      initDebug(instance, debugExtensionTag, ::debugSeverity, ::debugType);
  ...
  if (debugExtensionTag == DebugObjectType::debugUtils) {
    VkDebugUtilsObjectNameInfoEXT object = {
        ...
        nullptr, // pNext
        VK_OBJECT_TYPE_INSTANCE, handleToUint64(instance), "instance"};
    ...
    vkSubmitDebugUtilsMessageEXT(
        instance, VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT,
        VK_DEBUG_UTILS_MESSAGE_TYPE_PERFORMANCE_BIT_EXT, &dumcd);
  } else if (debugExtensionTag == DebugObjectType::debugReport) {
    vkDebugReportMessageEXT(
        instance, VK_DEBUG_REPORT_PERFORMANCE_WARNING_BIT_EXT,
        VK_DEBUG_REPORT_OBJECT_TYPE_INSTANCE_EXT, (uint64_t)instance, __LINE__,
        uncoded, "Application", introMsg);
  }
#endif
  ...
  std::tie<uint32_t, uint32_t>(graphicsQueueFamily, presentQueueFamily) =
      getQueueFamilies(physicalDevice, surface);

  const VkPhysicalDeviceFeatures features =
      {}; // don't need any special feature for this demo
  const vector<const char *> deviceExtensions = {
      VK_KHR_SWAPCHAIN_EXTENSION_NAME};

  device =
      initDevice(physicalDevice, features, graphicsQueueFamily,
                 presentQueueFamily, requestedLayers, deviceExtensions);
graphicsQueue = getQueue(device, graphicsQueueFamily, 0);
presentQueue = getQueue(device, presentQueueFamily, 0);

  VkSurfaceFormatKHR surfaceFormat = getSurfaceFormat(physicalDevice, surface);
  VkRenderPass renderPass =
      initRenderPass(device, physicalDevice, surfaceFormat);

  auto vertexShaderCode = readFile("VulkanVertexShader.spv");
  auto fragShaderCode = readFile("VulkanFragmentShader.spv");
  ...
  auto createTextureResult = createTextureImage(
      "brick.texture.bmp", device, physicalDevice, commandPool, graphicsQueue);
  ...
  auto textureSampler = createTextureSampler(device);

  auto createUniformBufferResult = createUniformBuffer(device, physicalDevice);
  VkBuffer uniformBuffer = std::get<0>(createUniformBufferResult);
  uniformBufferMemory = std::get<1>(createUniformBufferResult);

  auto descriptorSet =
      createDescriptorSet(uniformBuffer, textureImageView, textureSampler,
  ...
  VkPipelineLayout pipelineLayout =
      initPipelineLayout(device, descriptorSetLayout);

  VkBuffer vertexBuffer =
      initBuffer(device, sizeof(decltype(cube)::value_type) * cube.size(),
                 VK_BUFFER_USAGE_VERTEX_BUFFER_BIT);
  const std::vector<VkMemoryPropertyFlags> memoryTypePriority{
      VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT |
          VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
          VK_MEMORY_PROPERTY_HOST_COHERENT_BIT, // preferably wanna device-side
                                                // memory that can be updated
                                                // from host without hassle
      VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
          VK_MEMORY_PROPERTY_HOST_COHERENT_BIT // guaranteed to allways be
                                               // supported
  };
  VkDeviceMemory vertexBufferMemory = initMemory<ResourceType::Buffer>(
      device, physicalDeviceMemoryProperties, vertexBuffer, memoryTypePriority);
  setVertexData(device, vertexBufferMemory,
                cube); // Writes throug memory map. Synchronization is implicit
                       // for any subsequent vkQueueSubmit batches.

  // might need synchronization if init is more advanced than this
  // VkResult errorCode = vkDeviceWaitIdle( device ); RESULT_HANDLER( errorCode,
  // "vkDeviceWaitIdle" );

  // place-holder swapchain dependent objects
  swapchain =
      VK_NULL_HANDLE; // has to be NULL -- signifies that there's no swapchain

  pipeline = VK_NULL_HANDLE; // has to be NULL for the case the app
                                        // ends before even first swapchain
  // vector<VkCommandBuffer> commandBuffers;

  // vector<VkSemaphore> imageReadySs;
  // vector<VkSemaphore> renderDoneSs;

  // workaround for validation layer "memory leak" + might also help the driver
  // to cleanup old resources this should not be needed for a real-word app,
  // because they are likely to use fences naturaly (e.g. responding to user
  // input ) read
  // https://github.com/KhronosGroup/Vulkan-LoaderAndValidationLayers/issues/1628
  const uint32_t maxInflightSubmissions =
      2; // more than 2 probably does not make much sense
  submissionNr =
      0; // index of the current submission modulo maxInflightSubmission
  ...
  const std::function<bool(void)> recreateSwapchain = [&]() {
    ...
    TODO("This may be triggered from many sources (e.g. WM_SIZE event, and "
         "VK_ERROR_OUT_OF_DATE_KHR too). Should prevent duplicate swapchain "
         "recreation.")
    ...
    VkExtent2D surfaceSize = {capabilities.currentExtent.width,
    ...
    // cleanup old
    this->oldImageReadySs = imageReadySs;
    imageReadySs.clear();
    if (oldSwapchain) {
      {
      ...
      // fences might be in unsignaled state, so kill them too to get fresh
      // signaled
      killFences(device, submissionFences);

      // semaphores might be in signaled state, so kill them too to get fresh
      // unsignaled
      killSemaphores(device, renderDoneSs);
      // kill imageReadySs later when oldSwapchain is destroyed

      // only reset + later reuse already allocated and create new only if
      // needed
      {
        VkResult errorCode = vkResetCommandPool(device, commandPool, 0);
        RESULT_HANDLER(errorCode, "vkResetCommandPool");
      }
      ...
      // kill oldSwapchain later, after it is potentially used by
    ...
    if (swapchainCreatable) {
      ...
      swapchain = initSwapchain(physicalDevice, device, surface, surfaceFormat,
                                capabilities, graphicsQueueFamily,
                                presentQueueFamily, oldSwapchain);
      ...
      createImage(
          device, physicalDevice, screenWidth, screenHeight, depthFormat,
      ...
      swapchainImageViews = initSwapchainImageViews(device, swapchainImages,
                                                    surfaceFormat.format);
      framebuffers = initFramebuffers(device, renderPass, depthImageView,
                                      swapchainImageViews, surfaceSize.width,
                                      surfaceSize.height);

      pipeline = initPipeline(device, physicalDeviceProperties.limits,
                              pipelineLayout, renderPass, vertexShader,
                              fragmentShader, vertexBufferBinding,
      ...
      acquireCommandBuffers(device, commandPool,
                            static_cast<uint32_t>(swapchainImages.size()),
                            commandBuffers);
      for (size_t i = 0; i < swapchainImages.size(); ++i) {
        beginCommandBuffer(commandBuffers[i]);
        recordBeginRenderPass(commandBuffers[i], renderPass, framebuffers[i],
                              clearValues.data(), surfaceSize.width,
                              surfaceSize.height);

        recordBindPipeline(commandBuffers[i], pipeline);
        recordBindVertexBuffer(commandBuffers[i], vertexBufferBinding,
                               vertexBuffer);
        updateUniformBuffer(uniformBufferMemory, device);
      ...
      // per https://github.com/KhronosGroup/Vulkan-Docs/issues/1150 need upto
      // swapchain-image count
      renderDoneSs = initSemaphores(device, swapchainImages.size());

      this->submissionFences = initFences(
          device, maxInflightSubmissions,
          VK_FENCE_CREATE_SIGNALED_BIT); // signaled fence means previous
                                         // execution finished, so we start
                                         // rendering presignaled
      submissionNr = 0;
    }
    ...
    if (oldSwapchain) {
      ...
      // per current spec, we can't really be sure these are not used :/ at
      // least kill them after the swapchain
  ...
  this->renderClosure = [&]() {
    assert(
        swapchain); // should be always true; should have yielded CPU if false

    // vkAcquireNextImageKHR produces unsafe semaphore that needs extra cleanup.
    // Track that with this variable.
    bool unsafeSemaphore = false;

    try {
      // remove oldest frame from being in flight before starting new one
      // refer to doc/, which talks about the cycle of how the synch primitives
      // are (re)used here
      {
        VkResult errorCode = vkWaitForFences(
            device, 1, &submissionFences[submissionNr], VK_TRUE, UINT64_MAX);
        RESULT_HANDLER(errorCode, "vkWaitForFences");
      }
      {
        VkResult errorCode =
            vkResetFences(device, 1, &submissionFences[submissionNr]);
        RESULT_HANDLER(errorCode, "vkResetFences");
      }

      unsafeSemaphore = true;
      uint32_t nextSwapchainImageIndex =
          getNextImageIndex(device, swapchain, imageReadySs[submissionNr]);
      unsafeSemaphore = false;

      submitToQueue(graphicsQueue, commandBuffers[nextSwapchainImageIndex],
                    imageReadySs[submissionNr],
                    renderDoneSs[nextSwapchainImageIndex],
                    submissionFences[submissionNr]);
      present(presentQueue, swapchain, nextSwapchainImageIndex,
              renderDoneSs[nextSwapchainImageIndex]);

      submissionNr = (submissionNr + 1) % maxInflightSubmissions;
    } catch (VulkanResultException ex) {
      if (ex.result == VK_SUBOPTIMAL_KHR ||
          ex.result == VK_ERROR_OUT_OF_DATE_KHR) {
  ...
  // per current spec, we can't really be sure these are not used :/ at least
  // kill them after the swapchain
  // https://github.com/KhronosGroup/Vulkan-Docs/issues/152
  ...
  logger << "ERROR: Terminated due to an uncaught VkResult exception: "
         << vkE.file << ":" << vkE.line << ":" << vkE.func << "() "
         << vkE.source << "() returned " << to_string(vkE.result) << std::endl;
  return EXIT_FAILURE;
} catch (const char *e) {
  logger << "ERROR: Terminated due to an uncaught exception: " << e
         << std::endl;
  return EXIT_FAILURE;
} catch (string e) {
  logger << "ERROR: Terminated due to an uncaught exception: " << e
  ...
  logger << "ERROR: Terminated due to an uncaught exception: " << e.what()
         << std::endl;
  return EXIT_FAILURE;
} catch (...) {
  logger << "ERROR: Terminated due to an unrecognized uncaught exception."
         << std::endl;
  return EXIT_FAILURE;
}
...
void endSingleTimeCommands(VkQueue graphicsQueue, VkCommandPool commandPool,
...
void copyBufferToImage(VkQueue graphicsQueue, VkCommandPool commandPool,
                       VkDevice device, VkBuffer buffer, VkImage image,
                       uint32_t width, uint32_t height) {
...
void VulkanRender::updateUniformBuffer(VkDeviceMemory uniformBufferMemory, VkDevice device) {
...
void transitionImageLayout(VkQueue graphicsQueue, VkCommandPool commandPool,
...
createTextureImage(const char *imagePath, VkDevice device,
  ...
  VkDeviceSize imageSize =
      rgbaSurface->w * rgbaSurface->h * 4; // 4 bytes per pixel (RGBA)
  ...
  createImage(device, physicalDevice, rgbaSurface->w, rgbaSurface->h,
              VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_TILING_OPTIMAL,
              VK_IMAGE_USAGE_TRANSFER_DST_BIT | VK_IMAGE_USAGE_SAMPLED_BIT,
              VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, textureImage,
              textureImageMemory);

  transitionImageLayout(graphicsQueue, commandPool, device, textureImage,
                        VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_LAYOUT_UNDEFINED,
                        VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL);
  copyBufferToImage(graphicsQueue, commandPool, device, stagingBuffer,
                    textureImage, static_cast<uint32_t>(rgbaSurface->w),
...
VkImageView createTextureImageView(VkDevice device, VkImage textureImage) {
  auto textureImageView = createImageView(
...
bool isLayerSupported(const char *layer,
  ...
  return std::any_of(supportedLayers.begin(), supportedLayers.end(),
                     isSupportedPred);
}
...
bool isExtensionSupported(
    const char *extension,
    const vector<VkExtensionProperties> &supportedExtensions) {
  const auto isSupportedPred =
      [extension](const VkExtensionProperties &prop) -> bool {
...
checkInstanceLayerSupport(const vector<const char *> &requestedLayers,
  ...
  for (const auto layer : requestedLayers) {
    if (isLayerSupported(layer, supportedLayers))
      compiledLayerList.push_back(layer);
    else
      logger << "WARNING: Requested layer " << layer
             << " is not supported. It will not be enabled." << std::endl;
  }
...
vector<const char *>
checkInstanceLayerSupport(const vector<const char *> &optionalLayers) {
  return checkInstanceLayerSupport(optionalLayers,
                                   enumerate<VkInstance, VkLayerProperties>());
}
...
getSupportedInstanceExtensions(const vector<const char *> &providingLayers) {
  ...
  for (const auto pl : providingLayers) {
    const auto providedExtensions =
        enumerate<VkInstance, VkExtensionProperties>(pl);
    supportedExtensions.insert(supportedExtensions.end(),
                               providedExtensions.begin(),
                               providedExtensions.end());
  }
...
vector<VkExtensionProperties>
getSupportedDeviceExtensions(const VkPhysicalDevice physDevice,
                             const vector<const char *> &providingLayers) {
  auto supportedExtensions = enumerate<VkExtensionProperties>(physDevice);

  for (const auto pl : providingLayers) {
    const auto providedExtensions =
        enumerate<VkExtensionProperties>(physDevice, pl);
    supportedExtensions.insert(supportedExtensions.end(),
                               providedExtensions.begin(),
                               providedExtensions.end());
  }
...
bool checkExtensionSupport(
  ...
  for (const auto extension : extensions) {
    if (!isExtensionSupported(extension, supportedExtensions)) {
      allSupported = false;
      logger << "WARNING: Requested extension " << extension
             << " is not supported. Trying to enable it will likely fail."
             << std::endl;
    }
  }
...
bool checkDeviceExtensionSupport(const VkPhysicalDevice physDevice,
                                 const vector<const char *> &extensions,
                                 const vector<const char *> &providingLayers) {
  return checkExtensionSupport(
      extensions, getSupportedDeviceExtensions(physDevice, providingLayers));
}
...
VkInstance initInstance(const vector<const char *> &layers,
...
#if VULKAN_VALIDATION
  // in effect during vkCreateInstance and vkDestroyInstance duration (because
  // callback object cannot be created without instance)
  ...
  bool debugUtils =
      std::find_if(extensions.begin(), extensions.end(), [](const char *e) {
        return std::strcmp(e, VK_EXT_DEBUG_UTILS_EXTENSION_NAME) == 0;
      }) != extensions.end();
  bool debugReport =
      std::find_if(extensions.begin(), extensions.end(), [](const char *e) {
        return std::strcmp(e, VK_EXT_DEBUG_REPORT_EXTENSION_NAME) == 0;
      }) != extensions.end();
  if (!debugUtils && !debugReport)
    throw "VULKAN_VALIDATION is enabled but neither VK_EXT_debug_utils nor "
          "VK_EXT_debug_report extension is being enabled!";
  const void *debugpNext = debugUtils ? (void *)&debugUtilsCreateInfo
                                      : (void *)&debugReportCreateInfo;
#endif
...
bool isPresentationSupported(const VkPhysicalDevice physDevice,
                             const uint32_t queueFamily,
                             const VkSurfaceKHR surface) {
  VkBool32 supported;
  const VkResult errorCode = vkGetPhysicalDeviceSurfaceSupportKHR(
      physDevice, queueFamily, surface, &supported);
  RESULT_HANDLER(errorCode, "vkGetPhysicalDeviceSurfaceSupportKHR");
...
bool isPresentationSupported(const VkPhysicalDevice physDevice,
  ...
  for (uint32_t qf = 0; qf < qfCount; ++qf) {
    if (isPresentationSupported(physDevice, qf, surface))
      return true;
  }
...
VkPhysicalDevice getPhysicalDevice(const VkInstance instance,
                                   const VkSurfaceKHR surface) {
  ...
  if (surface) {
    for (auto it = devices.begin(); it != devices.end();) {
      ...
      if (!isPresentationSupported(pd, surface))
        it = devices.erase(it);
      else
        ++it;
    }
  ...
  if (devices.empty())
    throw string("ERROR: No Physical Devices (GPUs) ") +
        (surface ? "with presentation support " : "") + "detected!";
    ...
    for (const auto pd : devices) {
      ...
      if (pdp.deviceType == VK_PHYSICAL_DEVICE_TYPE_DISCRETE_GPU) {
        ...
        // __LINE__, 					1, u8"application", u8"More than one Physical Devices (GPU)
        // found. Choosing the first dedicated one."
        // 				);
        // #endif
...
#if VULKAN_VALIDATION
    vkDebugReportMessageEXT(instance, VK_DEBUG_REPORT_WARNING_BIT_EXT,
                            VK_DEBUG_REPORT_OBJECT_TYPE_INSTANCE_EXT,
                            handleToUint64(instance), __LINE__, 1,
                            "application",
                            "More than one Physical Devices (GPU) found. "
...
getQueueFamilies(const VkPhysicalDevice physDevice,
                 const VkSurfaceKHR surface) {
  constexpr uint32_t notFound = VK_QUEUE_FAMILY_IGNORED;
  const auto qfps = getQueueFamilyProperties(physDevice);
  const auto findQueueFamilyThat =
      [&qfps, notFound](
          std::function<bool(const VkQueueFamilyProperties &, const uint32_t)>
              predicate) -> uint32_t {
    for (uint32_t qf = 0; qf < qfps.size(); ++qf)
      if (predicate(qfps[qf], qf))
        return qf;
    return notFound;
  };

  const auto isGraphics = [](const VkQueueFamilyProperties &props,
                             const uint32_t = 0) {
    return props.queueFlags & VK_QUEUE_GRAPHICS_BIT;
  };
  const auto isPresent = [=](const VkQueueFamilyProperties &,
                             const uint32_t queueFamily) {
    return isPresentationSupported(physDevice, queueFamily, surface);
  };
  const auto isFusedGraphicsAndPresent =
      [=](const VkQueueFamilyProperties &props, const uint32_t queueFamily) {
  ...
  if (::forceSeparatePresentQueue) {
    ...
    const auto isSeparatePresent =
        [graphicsQueueFamily, isPresent](const VkQueueFamilyProperties &props,
                                         const uint32_t queueFamily) {
          return queueFamily != graphicsQueueFamily &&
                 isPresent(props, queueFamily);
        };
  ...
  if (graphicsQueueFamily == notFound)
    throw "Cannot find a graphics queue family!";
  if (presentQueueFamily == notFound)
    throw "Cannot find a presentation queue family!";

  return std::make_pair(graphicsQueueFamily, presentQueueFamily);
}

VkDevice initDevice(const VkPhysicalDevice physDevice,
                    const VkPhysicalDeviceFeatures &features,
  ...
  if (presentQueueFamily != graphicsQueueFamily) {
    queues.push_back({VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO,
                      nullptr, // pNext
                      0,       // flags
                      presentQueueFamily,
                      1, // queue count
...
VkQueue getQueue(const VkDevice device, const uint32_t queueFamily,
...
getMemoryRequirements<ResourceType::Buffer>(VkDevice device, VkBuffer buffer) {
...
template <>
VkMemoryRequirements getMemoryRequirements<ResourceType::Image>(VkDevice device,
                                                                VkImage image) {
  VkMemoryRequirements memoryRequirements;
...
template <ResourceType resourceType, class T>
void bindMemory(VkDevice device, T buffer, VkDeviceMemory memory,
                VkDeviceSize offset);

template <>
void bindMemory<ResourceType::Buffer>(VkDevice device, VkBuffer buffer,
                                      VkDeviceMemory memory,
                                      VkDeviceSize offset) {
  VkResult errorCode = vkBindBufferMemory(device, buffer, memory, offset);
  RESULT_HANDLER(errorCode, "vkBindBufferMemory");
}

template <>
void bindMemory<ResourceType::Image>(VkDevice device, VkImage image,
                                     VkDeviceMemory memory,
                                     VkDeviceSize offset) {
  VkResult errorCode = vkBindImageMemory(device, image, memory, offset);
...
initMemory(VkDevice device,
  ...
  const VkMemoryRequirements memoryRequirements =
      getMemoryRequirements<resourceType>(device, resource);
  ...
  if (memoryType == memoryTypeNotFound)
...
VkImageView initImageView(VkDevice device, VkImage image, VkFormat format,
  ...
  iciv.subresourceRange.aspectMask = aspectFlags;
...
VkSurfaceFormatKHR getSurfaceFormat(VkPhysicalDevice physicalDevice,
  ...
  vector<VkSurfaceFormatKHR> formats =
      enumerate<VkSurfaceFormatKHR>(physicalDevice, surface);
  ...
  if (chosenFormat1.format)
    return chosenFormat1;
  else if (chosenFormat2.format)
    return chosenFormat2;
  else
    return formats[0];
}
...
VkSurfaceCapabilitiesKHR getSurfaceCapabilities(VkPhysicalDevice physicalDevice,
  ...
  RESULT_HANDLER(errorCode, "vkGetPhysicalDeviceSurfaceCapabilitiesKHR");
...
VkPresentModeKHR getSurfacePresentMode(VkPhysicalDevice physicalDevice,
  ...
  vector<VkPresentModeKHR> modes =
      enumerate<VkPresentModeKHR>(physicalDevice, surface);

  for (auto m : modes) {
    if (m == ::presentMode) {
      if (selectedMode != 0) {
        logger << "INFO: Your preferred present mode became supported. "
                  "Switching to it.\n";
      }
  ...
  for (auto m : modes) {
    if (m == VK_PRESENT_MODE_FIFO_KHR) {
      if (selectedMode != 1) {
        logger << "WARNING: Your preferred present mode is not supported. "
                  "Switching to VK_PRESENT_MODE_FIFO_KHR.\n";
      }
  ...
  if (modes.empty())
    throw "Bugged driver reports no supported present modes.";
  else {
    if (selectedMode != 2) {
      logger << "WARNING: Bugged drivers. VK_PRESENT_MODE_FIFO_KHR not "
                "supported. Switching to whatever is.\n";
    }
...
VkSwapchainKHR initSwapchain(VkPhysicalDevice physicalDevice, VkDevice device,
  ...
  if (capabilities.supportedCompositeAlpha & VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR)
    ...
    throw "Unknown composite alpha reported.";

  // minImageCount + 1 seems a sensible default. It means 2 images should always
  // be readily available without blocking. May lead to memory waste though if
  // we care about that.
  uint32_t myMinImageCount = capabilities.minImageCount + 1;
  if (capabilities.maxImageCount)
    myMinImageCount =
        std::min<uint32_t>(myMinImageCount, capabilities.maxImageCount);
  ...
  VkSwapchainCreateInfoKHR swapchainInfo{
      ...
      // It should be fine to just use CONCURRENT in the off chance we encounter
      // the elusive GPU with separate present queue
      queueFamilies.size() > 1 ? VK_SHARING_MODE_CONCURRENT
                               : VK_SHARING_MODE_EXCLUSIVE,
      static_cast<uint32_t>(queueFamilies.size()), queueFamilies.data(),
      capabilities.currentTransform, compositeAlphaFlag,
...
vector<VkImageView> initSwapchainImageViews(VkDevice device,
  ...
  for (auto image : images) {
    VkImageView imageView =
        initImageView(device, image, format, VK_IMAGE_ASPECT_COLOR_BIT);
...
VkRenderPass initRenderPass(VkDevice device, VkPhysicalDevice physicalDevice,
  ...
  // implicitly defined dependency would cover this, but let's replace it with
  // this explicitly defined dependency!
...
VkPipeline initPipeline(VkDevice device, VkPhysicalDeviceLimits limits,
  ...
  VkPipelineShaderStageCreateInfo shaderStageStates[] = {
      {
          ...
          nullptr // SpecializationInfo - constants pushed to shader on pipeline
                  // creation time
      },
      {
          VK_STRUCTURE_TYPE_PIPELINE_SHADER_STAGE_CREATE_INFO,
  ...
  if (vertexBufferBinding > limits.maxVertexInputBindings) {
    throw string(
        "Implementation does not allow enough input bindings. Needed: ") +
  ...
  if (vertexBufferStride > limits.maxVertexInputBindingStride) {
    throw string(
        "Implementation does not allow big enough vertex buffer stride: ") +
        to_string(vertexBufferStride) + string(", max: ") +
  ...
  vector<VkVertexInputBindingDescription> inputBindingDescriptions = {
  ...
  VkVertexInputAttributeDescription positionInputAttributeDescription{
      positionLocation, vertexBufferBinding, VK_FORMAT_R32G32B32_SFLOAT,
      offsetof(Vertex3D_UV, position)};

  VkVertexInputAttributeDescription colorInputAttributeDescription{
      colorLocation, vertexBufferBinding, VK_FORMAT_R32G32B32_SFLOAT,
      offsetof(Vertex3D_UV, uv)};

  vector<VkVertexInputAttributeDescription> inputAttributeDescriptions = {
      positionInputAttributeDescription, colorInputAttributeDescription};
  ...
  VkPipelineColorBlendStateCreateInfo colorBlendState{
      ...
      VK_FALSE, // logic ops
      VK_LOGIC_OP_COPY,
      1, // attachment count - must be same as color attachment count in
         // renderpass subpass!
  ...
  VkGraphicsPipelineCreateInfo pipelineInfo{
      ...
      &rasterizationState,
      &multisampleState,
      &depthStencilState, // depth stencil
      &colorBlendState,
...
void setVertexData(VkDevice device, VkDeviceMemory memory,
  ...
  setMemoryData(device, memory, vertices.data(),
...
vector<VkSemaphore> initSemaphores(VkDevice device, size_t count) {
  ...
  std::generate_n(std::back_inserter(semaphores), count,
                  [device] { return initSemaphore(device); });
  return semaphores;
}
...
VkFence initFence(const VkDevice device, const VkFenceCreateFlags flags = 0) {
  ...
  VkFence fence;
  VkResult errorCode = vkCreateFence(device, &fci, nullptr, &fence);
  RESULT_HANDLER(errorCode, "vkCreateFence");
  return fence;
}
...
vector<VkFence> initFences(const VkDevice device, const size_t count,
                           const VkFenceCreateFlags flags) {
  vector<VkFence> fences;
  std::generate_n(std::back_inserter(fences), count,
                  [=] { return initFence(device, flags); });
  return fences;
}
...
void acquireCommandBuffers(VkDevice device, VkCommandPool commandPool,
  ...
  if (count > oldSize) {
    ...
    VkResult errorCode = vkAllocateCommandBuffers(device, &commandBufferInfo,
                                                  &commandBuffers[oldSize]);
    RESULT_HANDLER(errorCode, "vkAllocateCommandBuffers");
  }

  if (count < oldSize) {
    vkFreeCommandBuffers(device, commandPool, oldSize - count,
...
void beginCommandBuffer(VkCommandBuffer commandBuffer) {
  ...
  VkResult errorCode = vkBeginCommandBuffer(commandBuffer, &commandBufferInfo);
  RESULT_HANDLER(errorCode, "vkBeginCommandBuffer");
}

void endCommandBuffer(VkCommandBuffer commandBuffer) {
  VkResult errorCode = vkEndCommandBuffer(commandBuffer);
  RESULT_HANDLER(errorCode, "vkEndCommandBuffer");
}
...
void recordBindVertexBuffer(VkCommandBuffer commandBuffer,
  ...
  vkCmdBindVertexBuffers(commandBuffer, vertexBufferBinding,
                         1 /*binding count*/, &vertexBuffer, offsets);
}

void recordDraw(VkCommandBuffer commandBuffer, const uint32_t vertexCount) {
  vkCmdDraw(commandBuffer, vertexCount, 1 /*instance count*/,
...
void submitToQueue(VkQueue queue, VkCommandBuffer commandBuffer,
                   VkSemaphore imageReadyS, VkSemaphore renderDoneS,
                   VkFence fence) {
  const VkPipelineStageFlags psw =
  ...
  const VkResult errorCode =
      vkQueueSubmit(queue, 1 /*submit count*/, &submit, fence);
...
void present(VkQueue queue, VkSwapchainKHR swapchain,
  ...
  const VkResult errorCode = vkQueuePresentKHR(queue, &presentInfo);
  RESULT_HANDLER(errorCode, "vkQueuePresentKHR");
}

// cleanup dangerous semaphore with signal pending from vkAcquireNextImageKHR
// (tie it to a specific queue)
...
void cleanupUnsafeSemaphore(VkQueue queue, VkSemaphore semaphore) {
  ...
  const VkSubmitInfo submit{
      ...
      nullptr, // pNext
      1,
      &semaphore, // wait semaphores
      &psw,       // pipeline stages to wait for semaphore
      0,
      nullptr, // command buffers
      0,
      nullptr // signal semaphores
  };

  const VkResult errorCode =
      vkQueueSubmit(queue, 1 /*submit count*/, &submit, VK_NULL_HANDLE);
  RESULT_HANDLER(errorCode, "vkQueueSubmit");
}
...
void VulkanRender::render() {
    if (!initialized) {
        ...
        this->renderClosure();
        this->updateUniformBuffer(uniformBufferMemory, device);
    }
}
