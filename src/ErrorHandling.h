...
struct VulkanResultException {
  ...
  VulkanResultException(const char *file, unsigned line, const char *func,
                        const char *source, VkResult result)
      : file(file), line(line), func(func), source(source), result(result) {}
};

#define RESULT_HANDLER(errorCode, source)                                      \
  if (errorCode)                                                               \
  throw VulkanResultException(__FILE__, __LINE__, __func__, source, errorCode)
#define RESULT_HANDLER_EX(cond, errorCode, source)                             \
  if (cond)                                                                    \
  throw VulkanResultException(__FILE__, __LINE__, __func__, source, errorCode)

#define RUNTIME_ASSERT(cond, source)                                           \
  if (!(cond))                                                                 \
    throw source " failed";
...
void genericDebugCallback(std::string flags, Highlight highlight,
                          std::string msgCode, std::string object,
                          const char *message);
...
DebugObjectVariant
initDebug(const VkInstance instance, const DebugObjectType debugExtension,
...
translateFlags(const VkDebugUtilsMessageSeverityFlagsEXT debugSeverity,
               const VkDebugUtilsMessageTypeFlagsEXT debugType);
...
VKAPI_ATTR VkBool32 VKAPI_CALL genericDebugReportCallback(
  ...
  genericDebugCallback(
      dbrflags_to_string(flags), highlight,
      string(pLayerPrefix) + ", " + to_string(messageCode),
      to_string(objectType) + "(" + to_string_hex(object) + ")", pMessage);
...
VKAPI_ATTR VkBool32 VKAPI_CALL genericDebugUtilsCallback(
  ...
  for (uint32_t i = 0; i < pCallbackData->objectCount; ++i) {
    ...
    if (first)
      ...
      objects += ", ";
    objects +=
        to_string(obj.objectType) + "(" + to_string_hex(obj.objectHandle) + ")";
  }
  objects = "[" + objects + "]";

  genericDebugCallback(dbutype_to_string(messageTypes) + "+" +
                           to_string(messageSeverity),
                       highlight,
                       string(pCallbackData->pMessageIdName) + "(" +
                           to_string(pCallbackData->messageIdNumber) + ")",
                       objects, pCallbackData->pMessage);
...
translateFlags(const VkDebugUtilsMessageSeverityFlagsEXT debugSeverity,
  ...
  if ((debugType & VK_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT) ||
      (debugType & VK_DEBUG_UTILS_MESSAGE_TYPE_VALIDATION_BIT_EXT)) {
...
initDebug(const VkInstance instance, const DebugObjectType debugExtension,
  ...
  if (debugExtension == DebugObjectType::debugUtils) {
    ...
    const VkResult errorCode = vkCreateDebugUtilsMessengerEXT(
        instance, &dmci, nullptr, &debug.debugUtilsMessenger);
    RESULT_HANDLER(errorCode, "vkCreateDebugUtilsMessengerEXT");
  } else if (debugExtension == DebugObjectType::debugReport) {
    const VkDebugReportCallbackCreateInfoEXT debugCreateInfo{
        VK_STRUCTURE_TYPE_DEBUG_REPORT_CALLBACK_CREATE_INFO_EXT,
        nullptr, // pNext
        translateFlags(debugSeverity, debugType), ::genericDebugReportCallback,
        nullptr // pUserData
    };

    const VkResult errorCode = vkCreateDebugReportCallbackEXT(
        instance, &debugCreateInfo, nullptr, &debug.debugReportCallback);
...
void killDebug(const VkInstance instance, const DebugObjectVariant debug) {
  if (debug.tag == DebugObjectType::debugUtils) {
    vkDestroyDebugUtilsMessengerEXT(instance, debug.debugUtilsMessenger,
                                    nullptr);
  } else if (debug.tag == DebugObjectType::debugReport) {
    vkDestroyDebugReportCallbackEXT(instance, debug.debugReportCallback,
                                    nullptr);
  } else {
    throw "initDebug: unknown debug extension";
  }
