...
inline uint64_t handleToUint64(const PHANDLE_T *h) {
...
const char *to_string(const VkResult r) {
  ...
  default:
    return "unrecognized VkResult code";
  }
}
...
std::string
dbuseverity_to_string(const VkDebugUtilsMessageSeverityFlagsEXT debugSeverity) {
  std::string res;
  bool first = true;
  ...
  VkDebugUtilsMessageSeverityFlagsEXT known =
...
std::string
to_string(const VkDebugUtilsMessageSeverityFlagBitsEXT debugSeverity) {
